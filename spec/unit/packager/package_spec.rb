...
module Buildpack
  module Packager
    describe Package do
      ...
      describe 'with a specific stack' do
        let(:stack) { "cflinuxfs2" }
        let(:cflinuxfs2_dependency) { double(:cflinuxfs2_dependency) }
        let(:dependencies) { [dependency, cflinuxfs2_dependency] }
        ...
        describe '#download_dependencies' do
          ...
          before do
            allow(cflinuxfs2_dependency).to receive(:[])
            allow(cflinuxfs2_dependency).to receive(:fetch).with(:cf_stacks, []).and_return(['cflinuxfs2'])
            allow(cflinuxfs2_dependency).to receive(:[]).with('uri').and_return('file:///fake_uri_cflinuxfs2.tgz')

            allow(dependency).to receive(:[])
            allow(dependency).to receive(:fetch).with(:cf_stacks, []).and_return([])
          ...
          context 'before dependency has been cached locally' do
            ...
            it 'downloads the dependency to the local cache' do
              expanded_local_file_location = File.expand_path(File.join('local_cache_dir', 'file____fake_uri_cflinuxfs2.tgz'))
              expect(packager).to receive(:download_file).with('file:///fake_uri_cflinuxfs2.tgz', expanded_local_file_location)
              packager.download_dependencies(dependencies, local_cache_dir, dependency_dir)
            end
