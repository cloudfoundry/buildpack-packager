  ...
  context 'building the cached buildpack' do
    ...
    context 'with a buildpack packager dependency cache intact' do
      ...
      it 'outputs the dependencies downloaded, their versions, and cache location' do
        expect(subject).to match(/Used go version 1.6.1 from local cache at: .*.buildpack-packager\/cache\/https___storage.googleapis.com_golang_go1.6.1.linux-amd64.tar.gz/)
        expect(subject).to match(/Used go version 1.6.2 from local cache at: .*.buildpack-packager\/cache\/https___storage.googleapis.com_golang_go1.6.2.linux-amd64.tar.gz/)
        expect(subject).to match(/Used godep version v74 from local cache at: .*.buildpack-packager\/cache\/https___pivotal-buildpacks.s3.amazonaws.com_concourse-binaries_godep_godep-v74-linux-x64.tgz/)
      end
    end
  end
end
